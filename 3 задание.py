...
def searсh_path(data, x, y, short_path=None, full_path=None,
    ...
    if x == 3 and y == 8:  # Точка выхода из лабиринта
    ...
    for walk_X, walk_Y in walks:
        if data[x + walk_X][y + walk_Y] == 0 and (0 < x + walk_X < 9 and 0 < y + walk_Y < 9):  # Если ячейка свободна
            ...
            if check != None and check > count:  # Если мы были уже в этой точке и расстояние до неё больше, чем номер шага
                # на данном этапе
                full_path[(x + walk_X, y + walk_Y)] = count  # Перезаписываем full_path, т.к нашли более короткую дистанцию
...
def short_path(data, path=[], start=(3, 0), end=(3, 8)):  # Сюда прилетает short_path из search_path, когда нашли выход
